...
export default function PetDetailsImgageGallery({ pet }: ImgGalleryProps) {
  ...
  const mainAvatar = mainImg || (isValidImageUrl(pet?.avatar) ? pet.avatar : noImg);
  ...
  return (
    <>
      ...
      <div className='flex w-full items-center justify-between 2xl:h-full 2xl:w-40 2xl:flex-col'>
        <Image
          src={isValidImageUrl(pet?.avatar) ? pet.avatar : noImg}
          width={150}
          height={150}
        ...
        {Array.from({ length: 3 }).map((_, index) => {
          const imgUrl = pet?.images?.[index]?.imageUrl;
          const src = isValidImageUrl(imgUrl) ? imgUrl! : noImg;
          return (
            <Image
              key={index}
              src={src}
